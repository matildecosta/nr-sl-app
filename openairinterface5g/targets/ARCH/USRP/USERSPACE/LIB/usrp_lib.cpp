...
extern "C" {
  ...
  int device_init(openair0_device *device, openair0_config_t *openair0_cfg) {
    ...
    // JHNOTE 
    LOG_I(PHY, "openair0_cfg[0].rx_freq == '%f'\n", openair0_cfg[0].rx_freq[0]);
    LOG_I(PHY, "openair0_cfg[0].tx_freq == '%f'\n", openair0_cfg[0].tx_freq[0]);
    LOG_I(PHY, "openair0_cfg[0].rx_bw == '%f'\n", openair0_cfg[0].rx_bw);
	...
	LOG_I(HW,"Setting timing source to external\n");
      ...
      for(int i=0; i<s->usrp->get_rx_num_channels(); i++) {
        if (i<openair0_cfg[0].rx_num_channels) {
          ...
          // JHNOTE
          LOG_I(PHY, "D2D openair0_cfg[0].rx_freq == '%u'\n", openair0_cfg[0].rx_freq[i]);
          LOG_I(PHY, "USRP rx_freq == '%f'\n", s->usrp->get_rx_freq(i));
	  ...
	  if (device_adds.size()==2 && device->type == USRP_B200_DEV) {
	    ...
	    s2->usrp->set_tx_gain(gain_range_tx.stop()-openair0_cfg[0].tx_gain[i],i);
          ...
          // JHNOTE
          LOG_I(PHY, "D2D openair0_cfg[0].tx_freq == '%u'\n", openair0_cfg[0].tx_freq[i]); 
          LOG_I(PHY, "USRP tx_freq == '%f'\n", s->usrp->get_tx_freq(i));
        }
      }
