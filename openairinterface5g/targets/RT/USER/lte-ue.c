...

{
  ...
  // In phy_stub_UE (MAC-to-MAC) mode these init functions don't need to get called.
  if (nfapi_mode!=3)
    {
...
void init_UE(int nb_inst,int eMBMS_active, int uecap_xer_in, int timing_correction,int phy_test, int UE_scan, int UE_scan_carrier, runmode_t mode,int rxgain,int txpowermax,LTE_DL_FRAME_PARMS *fp0, int sidelink_active,int SLonly,int isSynchRef,int slsynconly,int SLSCHtest) {
  ...
  for (inst=0;inst<nb_inst;inst++) {
	  ...
	  if (simL1flag == 0) PHY_vars_UE_g[inst][0] = init_ue_vars(fp0,inst,0,sidelink_active);
		  ...
		  PHY_vars_UE_g[inst][0]  = init_ue_vars(&RC.ru[0]->frame_parms,inst,0,sidelink_active);
	  }
...
void init_UE_stub_single_thread(int nb_inst,int eMBMS_active, int uecap_xer_in, char *emul_iface, int simL1) {
  ...
  for (inst=0;inst<nb_inst;inst++) {
    ...
    //Emulation capabilities for sidelink currently de-activated
  ...
  while (oai_exit==0) {
	...
	if (UE->UE_scan_carrier == 1) {
	  ...
	  if (abs(freq_offset) > 7500) {
	    ...
	    // Lines 795- 807 commented out in sidelink branch
	    FILE *fd;
	    if ((fd = fopen("rxsig_frame0.dat","w"))!=NULL) {
	      fwrite((void*)&UE->common_vars.rxdata[0][0],
		     sizeof(int32_t),
...
static void *UE_thread_synchSL(void *arg)
{
  ...
  do  {
    ...
    //JHNOTE
    //printf( "Scanning band %d, dl_min %"PRIu32", ul_min %"PRIu32"\n", current_band, eutra_bands[ind].dl_min,eutra_bands[ind].ul_min);
    printf( "Scanning band %d, dl_min %"PRIu64", ul_min %"PRIu64", dl_freq %"PRIu64"\n", current_band, eutra_bands[ind].dl_min,eutra_bands[ind].ul_min,UE->frame_parms.dl_CarrierFreq);
  ...
  //JHNOTE
  LOG_I( PHY, "[SCHED][UE] Check absolute frequency SL %"PRIu64"(oai_exit %d, rx_num_channels %d)\n", UE->frame_parms.ul_CarrierFreq, oai_exit, openair0_cfg[0].rx_num_channels);
...
void fill_ue_band_info(void) {
  ...
  for (i=0; i<bands_to_scan.nbands; i++) {
    ...
    for (j=0; j<sizeof (eutra_bands) / sizeof (eutra_bands[0]); j++) {
        //printf("eutra_bands %d ; %d and UE capable band : %lu \n",UE_EUTRA_Capability->rf_Parameters.supportedBandListEUTRA.list.count, eutra_bands[j].band, UE_EUTRA_Capability->rf_Parameters.supportedBandListEUTRA.list.array[i]->bandEUTRA);
